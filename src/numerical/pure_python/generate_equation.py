...
def guess_length(x):
	...
	for num in range(-6, 7):
		for den in range(1, 7):
			...
			if approx_equal(x, num*math.sqrt(2)/den):
				return str(num)+'*sqrt(2)/' + str(den)
...
def get_equations(positions, naming_mode='trig', substitute_root_3= False, exponent_symbol='**'):
	parameter_names = []
	if naming_mode == "cart":
	...
	if substitute_root_3:
		string = string.replace('sqrt(3)', 'root3')

	string = string.replace('^', exponent_symbol)
	return string

'''
apply U12 = - (1/r^3)*(m1.m2) +(3/r^5) * (m1.r)*(m2.r) 
'''
def get_term(r, R, A, B):
	return r + '**-5*(-3*('+R[0] +'*'+A[0]+'+' +R[1] +'*'+ A[1] +')*('+R[0] +'*'+B[0]+'+' +R[1] +'*'+ B[1] +')) +' \
	\
	+ r +  '**-3*('+ A[0]+'*'+B[0]+'+'+A[1]+'*'+B[1]+')'
...
if __name__ == '__main__':
	...
	print(get_equations(positions, naming_mode='trig', exponent_symbol = '**'))
	input()
